...
suppressPackageStartupMessages({
  ...
  library(permute)    # how(), blocked permutations
  library(tibble)
  library(dplyr)
...
run_sim_invariable <- function(seed, prob, d, bugs_mat, dist_bugs) {
  ...
  # data frame for permabrioche::PERMANOVA_repeat_measures:
  ...
  perm_adj_B <- permabrioche::PERMANOVA_repeat_measures(
  ...
  hajek <- permabrioche::hajek_repeat_measures(
    formula           = dist_bugs ~ nonsense_invariant_f,
    data              = data_hajek,
    bugs              = bugs_mat,
    blocking_variable = "subject",
...
run_sim_variable <- function(seed, prob, d, df_subject, bugs_mat) {
  ...
  # data for permabrioche::PERMANOVA_repeat_measures: subject + both covariates
